...
import marcWeiss.sTracker.component.helper.CoreException;
...
public class FletcherChecksum {
	 ...
	 *** 
	 * @return A 2-integer array containing the current values of the checksum
	 *         accumulator.
	 **/
	...
	/**
	 *** Adds the specified byte array to the current running checksum
	 * accumulator.
	 *** 
	 * @param b
	 *            The byte array to add to the current running checksum
	 *            accumulator.
	 **/
	...
	public static String getFletcherForString(String string) {
		FletcherChecksum fc = new FletcherChecksum();
		if (string != null) {
			fc.runningChecksum(string.getBytes());
			return Integer.toString(fc.getChecksumAsInt());
		} else throw new CoreException("no String to calculate checksum");
	}
